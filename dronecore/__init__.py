...
class Command:
    def __init__(self, ctype: CommandType = CommandType.CMD_NONE, amount: int = -1,
                 response: bool = True, result: CommandResult = CommandResult.RES_OK):
    ...
    def __str__(self):
        if self.amount > 0:
            return "Command {} : amount={}{} response={} result={}".format(
                self.ctype.name, self.amount, self.ctype.unit, self.response, self.result.name)
        else:
