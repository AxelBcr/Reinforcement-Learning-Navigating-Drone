...
"""
from dronecore import ADrone, ARoom, CommandType, CommandResult, DroneState, Position
...
class DroneVirtual(ADrone):
    ...
    def _move(self, direction: str, n: int):
        if self.state != DroneState.INFLIGHT:
            ...
            self.command.result = CommandResult.RES_NO
            self.display(
                "Warning : {} movement inapplicable : minimum movement is {} cm".format(direction, self.minMove))
        elif n > self.maxMove:
            self.command.response = False
            self.command.result = CommandResult.RES_NO
            self.display(
                "Warning : {} movement inapplicable : maximum movement is {} cm".format(direction, self.maxMove))
        else:
            self.command.response = True
            self.savePosition()
            tmp = Position()
